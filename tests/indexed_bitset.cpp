...
BOOST_AUTO_TEST_CASE(indexed_bitset1)
{
	...
	b.flip(7);
	b.set(10);
	BOOST_TEST(b.to<std::uint32_t>(), 0b100010001);
}
...
{
	indexed_bitset<index_range<0, 0>> b(0x0u);
	b.set(0);
	BOOST_TEST(b.test(0));
	b.reset(0);
	BOOST_TEST(!b.test(0));
...
BOOST_AUTO_TEST_CASE(safe_arg_init)
{
	...
	BOOST_TEST(b.to<std::uint64_t>() == 0b101u);
	BOOST_TEST(b.to<std::uint8_t>() == 0b101u);
...
{
	indexed_bitset<Color, interval<2, 3>> b{safe_arg<Color::Red, 2>(true),
	...
	BOOST_TEST(b.to<std::uint64_t>(), 0b011011);
